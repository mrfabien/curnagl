...
#SBATCH --output /work/FAC/FGSE/IDYST/tbeucler/default/fabien/repos/curnagl/climatology/logs/con/con-%A_%a.out
#SBATCH --error /work/FAC/FGSE/IDYST/tbeucler/default/fabien/repos/curnagl/climatology/logs/error/err-%A_%a.err
...
python3 /work/FAC/FGSE/IDYST/tbeucler/default/fabien/repos/curnagl/climatology/extract_save_storms.py "$nom_var" "$annee" "$level"

# Print to a file a message that includes the current $SLURM_ARRAY_TASK_ID, the same variable, and the year of the sample
echo "This is array task ${SLURM_ARRAY_TASK_ID}, the variable name is ${nom_var} and the year is ${annee}. Level is ${level}" >> /work/FAC/FGSE/IDYST/tbeucler/default/fabien/repos/curnagl/climatology/logs/output_all.txt