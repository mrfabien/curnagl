...
for index, row in dates.iterrows():
    if row['start_date'].month >= 1 and row['start_date'].year == int(year):
...
for index, row in dates.iterrows():
    if row['end_date'].month <= 12 and row['end_date'].year == int(year_next):
...
for i in range(index_start_october,index_end_march):
    locals()['track_' + str(i+1)] = pd.read_csv('/work/FAC/FGSE/IDYST/tbeucler/default/fabien/repos/curnagl/tc_irad_tracks/tc_1_hour/tc_irad_' + str(i+1) + '_interp.txt')   
